...
from agents import issue_investigator, log_analyzer, solution_specialist, code_fix_generator
...
# Task 4: Generate code/config fixes based on the solution plan
generate_code_fix_task = Task(
    description="""Based on the log analysis, investigation, and solution plan, generate actual code fixes.

    Your code generation should:
    1. Write the exact code patches or configuration changes needed
    2. Include shell scripts or commands to apply the fix
    3. Provide updated configuration files (YAML, JSON, etc.) with the fix applied
    4. Add inline comments explaining each change
    5. Include a validation script to verify the fix works

    Output all code in proper code blocks with the correct language specified.
    Ensure all code is production-ready with error handling.""",
    expected_output="""A complete code fix package containing:
    - Code patches or updated configuration files
    - Shell script to apply the fix
    ...
    context=[analyze_logs_task, investigate_issue_task, provide_solution_task],
    output_file="task_outputs/code_fix.md",
)
